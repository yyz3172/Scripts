#!/bin/sh
# Mistral-7B-Instruct-v0.2/3P2_1D2_dynamickv_pa：3 个 P（各 2 卡 TP=2）+ 1 个 D（2 卡 TP=2），共 8 卡。
# 单脚本通过入参区分 Prefill 实例；与 run_decode.sh、pd_proxy.py 配套。
#
# 用法：
#   bash run_prefill.sh          # 后台启动 P0、P1、P2 并 wait（供 sweep / start_prefill 调用）
#   bash run_prefill.sh 0        # 仅 P0（卡 0,1，端口 9000）
#   bash run_prefill.sh 1        # 仅 P1（卡 2,3，端口 9001）
#   bash run_prefill.sh 2        # 仅 P2（卡 4,5，端口 9002）
...
if [ $# -eq 0 ]; then
    ...
    bash "$SCRIPT_DIR/run_prefill.sh" 2 &
    wait
else
    run_one "$1"
fi
