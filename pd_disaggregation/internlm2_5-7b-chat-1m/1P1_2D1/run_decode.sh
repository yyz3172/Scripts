#!/bin/sh
# internlm2_5-7b-chat-1m/1P1_2D1：单脚本通过入参区分 Decode 实例。与 run_prefill.sh 配套。
# 用法：
#   bash run_decode.sh          # 后台启动 D0、D1 并 wait（供 sweep / start_decode 调用）
#   bash run_decode.sh 0        # 仅启动第 1 个 D（卡 1，端口 9010）
#   bash run_decode.sh 1        # 仅启动第 2 个 D（卡 2，端口 9011）
# 日志：vllm 写入 ${LOG_DIR}/decode_<rank>.log（LOG_DIR 默认 .）；非法入参的提示仍打 stderr。
# 注：模型支持 1M 级上下文，但此处默认按 32k 先跑通；需要更长上下文可再调大。
...
model_path="/root/autodl-tmp/models/internlm2_5-7b-chat-1m"
...
# rank 0 -> 9010, 1；rank 1 -> 9011, 2
# ==========================================

run_one() {
    dp_rank=$1
    case $dp_rank in
        0) engine_port=9010; visible_devices="1" ;;
        1) engine_port=9011; visible_devices="2" ;;
    ...
    vllm serve "$model_path" \
        ...
        --seed 1024 \
        --served-model-name internlm2_5_7b_chat_1m \
        --dtype bfloat16 \
        --max-model-len 32768 \
