#!/bin/sh
# Mistral-7B-Instruct-v0.2/1P2_1D1：单机 1 个 Decode（1 卡 TP=1）。与 run_prefill.sh 配套。
# 日志：vllm 写入 ${LOG_DIR}/decode_0.log（LOG_DIR 默认 .）。
...
engine_port=9010
visible_devices="${DECODE_VISIBLE_DEVICES:-2}"
