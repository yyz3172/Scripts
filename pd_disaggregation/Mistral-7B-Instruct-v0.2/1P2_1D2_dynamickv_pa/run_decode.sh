#!/bin/sh
# Mistral-7B-Instruct-v0.2/1P2_1D2_dynamickv_pa：单机 1 个 Decode（2 卡 TP=2）。与 run_prefill.sh 配套。
# 开启 DynamicKV（--additional-config）+ Ascend PyTorch Profiler（--profiler-config）。
...
# DynamicKV 日志 profiling（配合 analysis/dynkv_profilers/ 解析 decode.log）
